"""Seed training modules, lessons, and achievements."""
import asyncio
import sys
...
async def seed_data():
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            result = await session.execute(select(TrainingModule))
            existing_modules = result.scalars().all()
            
            if existing_modules:
                logger.info("✅ Training modules already exist. Skipping seed.")
                return
            ...
            logger.info("✅ Created 7 training modules")
            
            # Create lessons for Module 1
            logger.info("Creating lessons for Module 1...")
            lessons = [
            ...
            logger.info(f"✅ Created {len(lessons)} lessons for Module 1")
            ...
            logger.info("\nSummary:")
            logger.info("  - 7 training modules")
            logger.info("  - 12 lessons for Module 1: Cybersecurity Fundamentals")
            logger.info("  - 14 achievements")
